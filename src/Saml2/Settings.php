...
class Settings
{
    ...
    private function _addDefaultValues(): void
    {
        ...
        if (!isset($this->_idp['singleSignOnService']['binding'])) {
            $this->_idp['singleSignOnService']['binding'] = Constants::BINDING_HTTP_REDIRECT;
        }
    ...
    public function checkIdPSettings(array $settings)
    {
        ...
        if (!isset($settings['idp']) || empty($settings['idp'])) {
            ...
            if (isset($settings['security'])) {
                ...
                if (!empty($settings['security']['nameIdEncrypted']) && !($existsX509 || $existsMultiX509Enc)
                ) {
